---
title: Products
subtitle: Check out our range of products
description: 'Precision astronomical products – focusers, sky sensors, AllSky cameras, and mount controllers. Open-source, designed for astronomers.'
keywords: 'telescope focuser, sky quality meter, mount controller, astronomical products, astrophotography equipment, telescope accessories, precision astronomy, open source hardware, AMFOC01, AMSKY01, AMSKY02, AMUSBMOUNT01'
layout: product-category
show_sidebar: false
hero_image: /images/am_products_hero.jpg
sections:
  - title: Products
    description: Precision astronomical instruments and devices designed for astrophotography, sky monitoring, and telescope control.
    collection: products
  - title: Components
    description: Small AstroMeters components for all-sky cameras, observatory builds, and custom astronomical projects.
    collection: accessories
  - title: Previous Generation
    description: Earlier product versions — still supported, now superseded by upgraded models.
    collection: legacy
---
